...
Describe "Get-TestRunSet function for $moduleName" -Tags Build {
    It "Should Return false when -WhatIf is used." {
        Get-TestRunSet -RestServer localhost -WhatIf | Should be $false
    }
    It "Should Return data when the call succeeds." {
        ...
        Get-TestRunSet -RestServer localhost -TestRunId 1 | Should not be $null
    ...
    It "Should not throw when the call succeeds." {
        ...
        {Get-TestRunSet -RestServer localhost -StatusId 1 } | Should not Throw
        Assert-MockCalled -CommandName 'Write-Log' -Times 1 -Exactly -Scope It
        Assert-MockCalled -CommandName 'Invoke-XesterRestCall' -Times 1 -Exactly -Scope It
    }
    It "Should throw when the Rest call fails." {
        ...
        {Get-TestRunSet -RestServer localhost -TestRun_Manager_ID 1} | Should Throw
        Assert-MockCalled -CommandName 'Write-Log' -Times 1 -Exactly -Scope It
        Assert-MockCalled -CommandName 'Invoke-XesterRestCall' -Times 1 -Exactly -Scope It
    }
}